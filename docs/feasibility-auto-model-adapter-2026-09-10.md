...
需求方细化后的流水线,把阶段④拆成**两级 LLM 分工** — 消化文档的模型 + 执行编码的 claude code:

```
本地探测新模型(免推理, models diff)
  → 联网搜索: 搜出新模型的调用方式/API 文档页        [搜索 key]
  → 抓取文档正文 → 已配置模型消化成"适配器制作指导"(结构化 spec)  [模型 key]
  → spec 作为编码委托交给 claude code → 写适配器
  → probe harness 验证数据通路 → 人审合入
```

- **两级优于单级**(对比 §5.2 直接喂 manifest): claude code 不啃原始文档省 context;spec 是人可审的中间产物;消化步骤可指定便宜长上下文模型。两个 key 的依赖就此闭环 — 搜索 key 找文档,模型 key 消化文档。
- **新缺口 — 文档正文抓取**: 搜索只给 URL+snippet,写 spec 需要正文。两条路: Tavily `extract` 端点(同族官方能力)或 Rust reqwest 直抓;API 文档站基本无反爬。约百行新增。
- **spec 模板**(消化模型的输出契约): 固定字段 — 协议归属(候选 T0/T1/T2)/端点与鉴权/请求参数表/响应结构/SSE 事件序列/与已知协议差异/适配器挂点(§1 清单)/验收标准。**只填字段不接受自由指令**,压缩注入面。
...
- **M3(维护者模式)**: §5A 两级流水线(搜索文档 → 抓正文 → 模型写 spec → claude code 编码 → harness 验证 → 人审);UI 放开发者设置区
